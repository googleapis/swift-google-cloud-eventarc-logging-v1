...
Add `swift-google-cloud-eventarc-logging-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-eventarc-logging-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudEventarcLoggingV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudEventarcLoggingV1 <target-name> --package swift-google-cloud-eventarc-logging-v1
```
